...
export default {
  ...
  computed: {
    ...mapState({
      ReturnedAssets: state => state.Assets.ReturnedAssets
    }),
    searchFilter: {
      get: function() {
        return this.ReturnedAssets.filter(product => {
          return (
  ...
  methods: {
    async LoadPosts() {
      this.$store.dispatch("RETURNED_ASSETS");
    },

    async paginate() {
      this.total_items = this.ReturnedAssets.length;
