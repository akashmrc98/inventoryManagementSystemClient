...
export default {
  ...
  computed: {
    ...mapState({
      unReturnedAssets: state => state.Assets.unReturnedAssets
    }),
    searchFilter: {
      get: function() {
        return this.unReturnedAssets.filter(product => {
          return (
  ...
  methods: {
    async LoadPosts() {
      this.$store.dispatch("PENDING_ASSETS");
    },

    async paginate() {
      this.total_items = this.unReturnedAssets.length;
